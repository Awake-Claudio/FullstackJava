...
import com.klaudecode.blogcms.mapper.ComentarioMapper;
...
public class ComentarioRepository implements ComentarioRep {
	...
	public boolean update(Comentario comentario) {
		if(comentario.getIdComentario()>0) {
			String sql = String.format("update Comentario set Comentario='%s' where IdComentario='%d'",
					comentario.getComentario(), comentario.getIdComentario());
			jdbcTemplate.execute(sql);
			return true;
		}
		return false;
