...
import com.klaudecode.blogcms.mapper.CategoriaMapper;
...
public class CategoriaRepository implements CategoriaRep {
	...
	public boolean save(Categoria categoria) {
		try {
			String sql = String.format(
					"insert into Categoria (Nombre,Descripcion) " //(Nombre,Descripcion,CategoriaSuperior)
					+ "values('%s', '%s')", //"values('%s', '%s', %d)"
					categoria.getNombre(), categoria.getDescripcion()); //categoria.getNombre(), categoria.getDescripcion(), categoria.getCategoriaSuperior());
	...
	public boolean update(Categoria categoria) {
		if(categoria.getIdCategoria() > 0) {
			String sql = String.format("update Categoria set Nombre='%s', Descripcion='%s'" //Nombre='%s', Descripcion='%s', CategoriaSuperior='%d' "
					+ "where IdCategoria='%d'", 
					categoria.getNombre(), categoria.getDescripcion(),  //categoria.getNombre(), categoria.getDescripcion(), categoria.getCategoriaSuperior(),
